...
__rcsId("$Id: main.cpp 99 2013-01-25 17:09:27Z mzbjyza2mji4zgz $")
...
protected:
    ...
    runParent() {
        int cIn = childInFd();
        if (write(cIn, mPasswd, mPasswdLen) != (ssize_t)mPasswdLen) {
            return 1;
        }
		...
		doWaitErr == true)) {
            ...
            if ((doLoop = waitChildReadable(out, err)) == true) {
                if (doWaitOut == true && out == true) {
                    if ((n = read(cOut, buf, sizeof(buf))) > 0) {
                        if (mVerbose == true) {
                ...
                if (doWaitErr == true && err == true) {
                    if ((n = read(cErr, buf, sizeof(buf))) > 0) {
                        if (mVerbose == true) {
