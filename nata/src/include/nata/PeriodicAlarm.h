/*
 * $Id: PeriodicAlarm.h 100 2013-05-24 13:16:43Z mzbjyza2mji4zgz $
 */
...
private:
    ...
    __rcsId("$Id: PeriodicAlarm.h 100 2013-05-24 13:16:43Z mzbjyza2mji4zgz $");
...
private:
    ...
    pStop(void) {
        ...
        size_t nPAHs = 0;
        size_t nDelPAHs = 0;
        PeriodicAlarmHandle **pahPtrs = NULL;
        bool dummy;
        bool ret = false;

        {
            ...
            if (mIsStopping == false) {
                ...
                nPAHs = mPATbl.size();
                if (nPAHs > 0) {
                    pahPtrs = (PeriodicAlarmHandle **)
                        malloc(sizeof(PeriodicAlarmHandle *) * nPAHs);
                    if (pahPtrs != NULL) {
                        nPAHs = mPATbl.getHandles(pahPtrs, nPAHs);
                        for (i = 0; i < nPAHs; i++) {
        ...
        while (nPAHs > nDelPAHs) {
            nDelPAHs = mDelPATbl.getHandles(pahPtrs, nPAHs);
...
public:
    ...
    acquireAlarm(size_t n = BOUNDEDBLOCKINGQUEUE_DEFAULT_MAX_LENGTH) {
        PeriodicAlarmHandle *ret = NULL;

        ScopedLock l(&mLock);

        if (mIsStopping == false) {
            ret = new PeriodicAlarmHandle(n);
            pPutHandle(ret);
        }
