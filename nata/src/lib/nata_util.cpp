...
__rcsId("$Id: nata_util.cpp 99 2013-01-25 17:09:27Z mzbjyza2mji4zgz $")
...
nata_ReadInt8(int fd, int8_t *buf, iolen_t len) {
    ...
    while ((iolen_t)ret < len) {
        n = read(fd, (void *)(buf + ret), len - (iolen_t)ret);
        if (n <= 0) {
            break;
        }
        ret += n;
...
nata_WriteInt8(int fd, const int8_t *buf, iolen_t len) {
    ...
    while ((iolen_t)ret < len) {
        n = write(fd, (void *)(buf + ret), len - (iolen_t)ret);
        if (n <= 0) {
            break;
        }
        ret += n;
...
nata_ReadInt64(int fd, int64_t *sPtr, iolen_t sLen) {
    ...
    while ((iolen_t)ret < sLen) {
        ...
        sPtr[ret++] = (int64_t)nata_ntohll(val);
    }
...
nata_WriteInt64(int fd, const int64_t *sPtr, iolen_t sLen) {
    ...
    while ((iolen_t)ret < sLen) {
        val = nata_htonll((uint64_t)sPtr[ret]);
        n = nata_WriteInt8(fd, (const int8_t *)&val, s);
        if ((iolen_t)n != s) {
            break;
        }
...
bool
nata_ParseBool(const char *str, _BOOL_ *val) {
...
#error Unknown/Non-supported API.
        ...
        int ret = -1;
        bool isAbsPath = (path[0] == '/') ? true : false;
        int nTokens;
        char *tokens[4096];
...
bool
nata_TTYSetBaudRate(int ttyfd, unsigned int rate) {
    struct termios t;
...
void
nata_NanoSleep(uint64_t nSec) {
    struct timespec to0;
    struct timespec to1;

    to0.tv_sec = (time_t)(nSec / (1000LL * 1000LL * 1000LL));
    to0.tv_nsec = (long)(nSec % (1000LL * 1000LL * 1000LL));

    reSleep:
    if (nanosleep(&to0, &to1) != 0) {
        if (errno == EINTR) {
            errno = 0;
            to0 = to1;
            goto reSleep;
